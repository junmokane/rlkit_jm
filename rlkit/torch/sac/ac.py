...
'''
AC with soft clipped double Q-learning
'''
class ACTrainer(TorchTrainer, LossFunction):
    def __init__(
            self,
            env,
            policy,
    ...
    def compute_loss(
        ...
        policy_loss = (-q_new_actions[:, 0]).mean()
        ...
        with torch.no_grad():
            ...
            next_dist = self.policy(state_rep)
            action_rep, new_log_pi = next_dist.rsample_and_logprob()
        ...
        qf1_loss = ((q1_pred - target_Q.detach()).pow(2)).mean()
        qf2_loss = ((q2_pred - target_Q.detach()).pow(2)).mean()
